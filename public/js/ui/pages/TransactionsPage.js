...
class TransactionsPage {
  ...
  registerEvents() {
    this.element.addEventListener('click', event => {
      ...
      if (removeButton) {
        if (removeButton.classList.contains('remove-account')) this.removeAccount();
        else if (removeButton.classList.contains('transaction__remove')) this.removeTransaction(removeButton.dataset.id);
      }
  ...
  render(options) {
    ...
    Transaction.list(options, (err, response) => {
      if (response.success) this.renderTransactions(response.data);
    })
  }
  ...
  formatDate(date) {
    ...
    return `${dateFormatter.format(dateObject)} в ${timeFormatter.format(dateObject)}`;
  }
  ...
  renderTransactions(data) {
    ...
    content.innerHTML = '';
    data.forEach(el => {
      content.innerHTML += this.getTransactionHTML(el);
    })
  }
}